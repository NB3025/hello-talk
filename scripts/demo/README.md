...
**이 장면은 워크샵 실습 재료다.** 원인을 찾고 고치는 것이 실습이므로 main에서는 이 동작을 바꾸지 않고, 원인 설명도 이 저장소에 적지 않는다. 고친 버전이 필요하면 별도 브랜치에 둔다.
...
- **`Connection: close`인 이유**: Chrome은 재사용 중인 keep-alive 소켓이 응답 없이 끊기면 POST라도 조용히 한 번 더 보내서 "실패"가 화면에 안 뜬다. 소켓 재사용을 막으면 끊김이 네트워크 오류로 드러난다.
...
두 번째 탭에서 로그인 화면의 "이미 있는 사람"으로 봇을 고르면, 첫 탭(화면상 강사)이 보낸 메시지가 봇 명의로 저장되어 왼쪽 말풍선으로 뜬다. 로그인 화면 문구가 "탭을 두 개 열어"라고 안내하므로 제품이 시키는 대로 하면 재현된다. 이것도 워크샵 재료라 그대로 둔다.
